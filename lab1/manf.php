...
function insertIntoAssetsTable()
{
    ...
    $sql = "insert into assets (asset_id,company_id,purch_date,purch_price,expire_date,comments) 
            values ('".$_SESSION['asset_id']."','".$_SESSION['company_id']."','".$_SESSION['purchaseDate']."','".
            $_SESSION['purchasePrice']."','".$_SESSION['Expire_Date']."','".$_SESSION['comments']."')";
...
function getManufcId()
{
    ...
    $sql = "SELECT company_id FROM support WHERE company_name = '".$name."' and web_page = '".$url."'";
    $stmt = $dbConn -> prepare ($sql);
    $stmt -> execute();
    $record = $stmt->fetch(PDO::FETCH_ASSOC);
...
function insertIntoSupport()
{
    ...
    $sql = "insert into support (company_name,address,phone,web_page) values
